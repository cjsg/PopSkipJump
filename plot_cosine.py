...
fig.suptitle('Cosine Analysis - Deterministic Model (1000 test images)'
                '\ntheta = g / sqrt(d)'
...
for r, raw in enumerate(raws):
    ...
    ax1.plot(range(1, 31), medians_line, label='g={:.2f}'.format(G[r]))
    ax2.plot(range(1, 31), medians_estm, label='g={:.2f}'.format(G[r]))
...
ax1.legend()
ax2.legend()
plt.savefig('adv/cos_true_1000_combined.pdf')
pass
